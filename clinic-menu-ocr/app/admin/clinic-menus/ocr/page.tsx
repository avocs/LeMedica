/**
 * Clinic Menu OCR → CSV Admin Tool
 *
 * This page allows internal admins to:
 * 1. Upload clinic package menus (PDF, images, or folders)
 * 2. Process them through OCR + AI extraction
 * 3. Preview and edit extracted packages in a table
 * 4. Regenerate CSV from edited data
 * 5. Download CSV or upload directly to bulk importer
 *
 * Integration:
 * - Place this file at: app/admin/clinic-menus/ocr/page.tsx
 * - Assumes existing admin layout wraps this page
 ...
 *
 * How to Test:
 * 1. Upload 1 PDF or image file, verify packages appear in table
 * 2. Edit a row, click "Regenerate CSV", download and open in Excel
 * 3. Toggle "Auto-create", click "Upload CSV to Admin", check toast
 * 4. Try editing JSON directly and applying changes
 * 5. Test validation by clearing required fields
 */
...
import { JsonPreviewPanel } from "@/components/ocr/json-preview-panel"
...
import { validateAllRows } from "@/lib/utils/validation"
...
export default function ClinicMenuOcrPage() {
  ...
  useEffect(() => {
    if (packages.length > 0) {
      ...
      // Update summary with client-side validation results
      if (summary) {
        setSummary({
          ...summary,
  ...
  const handleProcess = useCallback(async () => {
    ...
    try {
      ...
      setCsvBlob(null) // Reset CSV when new data comes in
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      ...
      <div className="container mx-auto px-4 py-6 space-y-6">
        ...
        {/* JSON Preview + Debug */}
        <div className="grid gap-6 lg:grid-cols-2">
          <JsonPreviewPanel packages={packages} batchId={batchId} onApplyJson={handleApplyJson} />
          <DebugPanel lastResponse={lastResponse} lastError={lastError} requestDuration={requestDuration} />
        </div>
      </div>
