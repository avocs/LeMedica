...
import type { PackageRow, FileInfo } from "@/lib/types/ocr"
...
export function OcrSummaryPanel({
  ...
  // Aggregate unique warnings
  const allWarnings = packages
    .flatMap((pkg) => pkg._meta?.warnings || [])
    .reduce((acc, warning) => {
          ...
          <Collapsible open={isWarningsOpen} onOpenChange={setIsWarningsOpen}>
            <CollapsibleTrigger asChild>
              <Button variant="ghost" className="w-full justify-between px-3 h-9">
                <span className="text-sm font-medium">Warnings ({allWarnings.size} unique)</span>
            ...
            <CollapsibleContent>
              <div className="mt-2 space-y-1 max-h-40 overflow-y-auto rounded-md border bg-amber-500/5 p-2">
                {Array.from(allWarnings.entries()).map(([warning, count]) => (
